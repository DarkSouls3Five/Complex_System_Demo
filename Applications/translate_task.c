...
/**
  ...
  * @retval         none
  */
//µç»úÒÆ¶¯Ò»²½µÄº¯Êı·â×°£¬²ÎÊı£º½á¹¹ÌåÖ¸Õë£¬µç»úÒÆ¶¯¾àÀë£¬µç»úĞòºÅ£¬µç»úËÙ¶È
...
float trans_distance[3];//¼ÇÂ¼µç»ú×´Ì¬ÇĞ»»Ç°µÄ¾ø¶Ô½Ç¶È
float trans_distance1,trans_distance2,trans_distance3;//debug¿´µÄ£¬Ã»É¶ÓÃ
uint8_t pos_A,pos_B,pos_C,pos_D;//¼ÇÂ¼¸÷ÇøÓòËù´¦µ²Î»

//Âß¼­ÅĞ¶ÏËùÉèÖÃµÄÒ»ÏµÁĞ²ÎÊı
float trans_move_angle;//ÉèÖÃµç»ú×ª¶¯¾àÀë
float trans_target_distance = 0.0f;//ÉèÖÃµç»úÒÆ¶¯µÄÄ¿±ê½Ç¶È

uint8_t trans_motor_number;//ÉèÖÃ½«ÒªÖ´ĞĞ¶¯×÷µÄµ¥µç»úĞòºÅ


uint8_t TRANS_DIR = DIR_L;//ÉèÖÃµç»ú×ª¶¯·½Ïò

uint8_t	last_infrared_return = 0;//Ç°Ò»´ÎºìÍâ±ä»¯Öµ
uint8_t	warning_flag = 0;//±¨¾¯ÌáÊ¾±êÊ¶
...
/**
  ...
  * @retval         none
  */
//½øĞĞÂß¼­ÅĞ¶Ï£¬Í¨¹ıºìÍâĞÅºÅÅĞ¶ÏÄÄ¸öÇøÓò×°Âú£¬²¢¾İ´ËÉèÖÃ¸÷µç»úµÄÒÆ¶¯ĞĞÎª
static void trans_set_param(trans_act_t *trans_act_param)
{
	
	//´ı²¹³äÍêÉÆ£¬Ö÷º¯ÊıÀïÔÚset_modeÇ°µ÷ÓÃ


/**************AÇø×°Âú£¬¸ù¾İµç»ú2ºÍ3Ëù´¦distanceÅĞ¶Ï²¢¿ØÖÆÒÆ¶¯***************/	
	if(last_infrared_return == 0 && infrared_return == 2)
	{
		//Í¨¹ıµç»ú2ºÍ3Î»ÖÃ½øĞĞÅĞ¶Ï£¬ÏÈÅĞ¶Ï2		
		if(trans_act_param->motor_data[1].trans_motor_measure->distance > 250.0f + TRANS_DISTANCE_ERR)
		//>250£¬×óÒÆ+errÏû³ıÒÆ¶¯Îó²î
		{
			//µç»ú2×óÒÆÖÁ250
			TRANS_DIR = DIR_L;
...
/**************BÇø×°Âú£¬¸ù¾İµç»ú1Ëù´¦distanceÅĞ¶Ï²¢¿ØÖÆÒÆ¶¯***************/		
	else if(last_infrared_return == 0 && infrared_return == 4)
	/*BÇø×°Âú£¬¸ù¾İµç»ú1Ëù´¦distanceÅĞ¶Ï²¢¿ØÖÆÒÆ¶¯*/
	{
		//Í¨¹ıµç»ú1Î»ÖÃ½øĞĞÅĞ¶Ï		
...
/**************CÇø×°Âú£¬¸ù¾İµç»ú3Ëù´¦distanceÅĞ¶Ï²¢¿ØÖÆÒÆ¶¯***************/		
	else if(last_infrared_return == 0 && infrared_return == 3)
	/*CÇø×°Âú£¬¸ù¾İµç»ú3Ëù´¦distanceÅĞ¶Ï²¢¿ØÖÆµç»ú2ºÍ3ÒÆ¶¯*/
	{
		//Í¨¹ıµç»ú3Î»ÖÃ½øĞĞÅĞ¶Ï£¬²¢¿ØÖÆµç»ú3ÒÆ¶¯		
			if(trans_act_param->motor_data[2].trans_motor_measure->distance > 250.0f + TRANS_DISTANCE_ERR)
			//>-250£¬×óÒÆ+errÏû³ıÒÆ¶¯Îó²î
			{
				//µç»ú3×óÒÆÖÁ-250
...
/**************DÇø×°Âú£¬¸ù¾İµç»ú1ºÍ2Ëù´¦distanceÅĞ¶Ï²¢¿ØÖÆÒÆ¶¯***************/	
	if(last_infrared_return == 0 && infrared_return == 1)
	{
		//Í¨¹ıµç»ú1ºÍ2Î»ÖÃ½øĞĞÅĞ¶Ï£¬ÏÈÅĞ¶Ï1		
		if(trans_act_param->motor_data[0].trans_motor_measure->distance > 250.0f + TRANS_DISTANCE_ERR)
		//>250£¬×óÒÆ+errÏû³ıÒÆ¶¯Îó²î
		{
			//µç»ú1×óÒÆÖÁ250
...
static void trans_set_mode(trans_act_t *trans_act_mode)
{
    if (trans_act_mode == NULL)
    {
		...
		if(garbage_mode.garbage_mode == MODE_WORK)
		{
			...
			//ÓĞÇøÓò×°Âú£¬¸ù¾İÉèÖÃµÄ²ÎÊıÅĞ¶Ï½ø×óÒÆ»¹ÊÇÓÒÒÆÄ£Ê½
			{
				if(TRANS_DIR == DIR_L)
